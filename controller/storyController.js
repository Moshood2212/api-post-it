...
const editStory = async (req, res) => {
    ...
    const { storyId } = req.params
    try {
        const story = await Story.findOneAndUpdate({ _id: storyId, createdBy: userId, }, req.body, { new: true, runValidators: true })
        res.status(201).json({success:true, story})
    } catch (error) {
        res.json(error)
    }
...
module.exports = {getAllStories,getAStory,getAllUserStories,getAUserStory,createStory,editStory,deleteStory}